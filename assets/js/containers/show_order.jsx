...
import {Accordion, Row, Tabs, Tab, Button, Table, Form, Card} from 'react-bootstrap';
    ...
    render() {
        ...
        if (!order) {
                ...
                <Card>
                    <Card.Body>
                        <Card.Header>
                            ...
                            <Card.Subtitle className="mb-2"><h3>Ordered on: {order.inserted_at.split("T")[0]}</h3></Card.Subtitle>
                            <Card.Subtitle className="mb-2"><h3>Order Status: {order.status}</h3></Card.Subtitle>
                            {order.tracking ? <div><Accordion.Toggle as={Button} variant="link" eventKey="0">
                                Tracking : {order.tracking}
                            </Accordion.Toggle>
                            <Accordion.Collapse eventKey="0">
                                <Card.Subtitle>Hello! I'm the body</Card.Subtitle>
                            </Accordion.Collapse>
                            </div>
                            : null}
                            <br />
                        ...
                        <Card.Text>
                            <Table>
                                <thead>
                                <tr>
                                    <th md={2} className="headings-display"></th>
                                    <th md={7} className="headings-display">Product Details</th>
                                    <th md={2} className="headings-display">Purchase Price</th>
                                </tr>
                                </thead>
                                <tbody>
                                {items}
            ...
            return (<div className={"offset-1 order-container col-md-10"}>
                ...
                let btn_message = {1: "Accept Order", 2: "Start Order Preparation", 3: "Dispatch Order", 4: "Item Delivered"}
                let updateTracking = (<div>
                    <h3>Process Order</h3>
                    <h4>Current Order Status: {order.status}</h4>
                    {order.status_id == 3 ? <Form.Control type="text" onChange={(event) => this.setState({tracking_num: event.target.value})} placeholder={"Enter Tracking/Reference Number"} /> : null}
                    {order.status_id < 5 ? <Button onClick={() => update_order_status(order.id, order.status_id+1, this.state.tracking_num)}>{btn_message[order.status_id]}</Button> : null}
                </div>)
                return(<Tabs defaultActiveKey="order" id="uncontrolled-tab-example">
